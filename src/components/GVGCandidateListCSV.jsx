...
export default function GVGCandidateListCSV({ data }) {
  ...
  return (
    <div className="candidate-list">
      {data.nodes.map((race, i) => (
        <div key={i}>
          {i === 0 && <h2 className="gvg-race green">{race.Race.replace('STATE', '')}<a name={race.Race.replace(/ /g, '')}></a></h2>}
          {i === 1 && <h2 className="gvg-race green">U.S. HOUSE OF REPRESENTATIVES</h2>}
          {race.Race === 'INDIANA STATE SENATE DISTRICT 4' && <h2 className="gvg-race green">INDIANA STATE SENATE</h2>}
          {race.Race === 'INDIANA STATE HOUSE DISTRICT 5' && <h2 className="gvg-race green">INDIANA STATE HOUSE</h2>}
          {race.Race === 'BARTHOLOMEW COUNTY COUNCIL DISTRICT 1' && <h2 className="gvg-race green">BARTHOLOMEW COUNTY</h2>}
          {race.Race === 'HAMILTON COUNTY COUNCIL DISTRICT 2' && <h2 className="gvg-race green">HAMILTON COUNTY</h2>}
          {race.Race === 'PORTER COUNTY COMMISSIONER DISTRICT 2' && <h2 className="gvg-race green">PORTER COUNTY COMM</h2>}
          {race.Race === 'TIPPECANOE COUNTY COUNCIL DISTRICT 4' && <h2 className="gvg-race green">TIPPECANOE COUNTY</h2>}
          {(race.Race.includes('U.S. HOUSE') || race.Race.includes('STATE')) && (
            <h3 className="gvg-race">{race.Race.replace('STATE', '')}<a name={race.Race.replace(/ /g, '')}></a></h3>
          )}
          {race.Race.includes('COUNTY') && (
            <h3 className="gvg-race">{race.Race.split(' ').slice(1).join(' ')}</h3>
          )}
