...
export default function GVGCandidateListPrimary({ data }) {
  if (!data?.primaries) return null;

  return (
    <div className="candidate-list">
      {data.primaries.map((primary, i) => (
        <div key={i}>
              ...
              {group.races.map((race, k) => (
                <div key={k} style={{ marginBottom: '40px' }}>
                  ...
                  <div className="gvg-race-row">
                    {race.candidates.map((candidate, l) => (
                      <div key={l} className="gvg-candidate-item" style={{ borderBottom: '1px solid black' }}>
                        <div className="gvg-column-2" style={{ width: '40%' }}>
                          <div className="gvg-name" style={{ padding: '10px' }}>{candidate.name}</div>
                          {(candidate.preferred || candidate.potential || candidate.badInfo) && (
                            <div className="gvg-preferred" style={{ paddingLeft: '10px', fontWeight: 'bold' }}>
                              ...
                              {candidate.badInfo && (
                                <div className="bad-candidate" style={{ display: 'flex', color: 'red' }}>
                                  <div style={{ paddingRight: '2px', fontSize: '32px' }}>X </div>
                                  <div className="badInfo">{candidate.badInfo}</div>
                                </div>
                              )}
                            </div>
                          )}
